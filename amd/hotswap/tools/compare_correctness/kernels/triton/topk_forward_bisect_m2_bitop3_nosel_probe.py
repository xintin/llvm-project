"""Probe post-compare boolean transforms without SEL data path."""
...
@triton.jit
def _topk_forward_bisect_m2_bitop3_nosel_probe(
    X, stride_xm,
    CMP, INV_BIT, INV_SUB, DELTA, stride_ym,
    n_rows, n_expts_tot,
    ...
    inv_bit = ((~cmp_u32) & 1).to(tl.uint32)
    inv_sub = (1 - cmp_u32).to(tl.uint32)
    delta = (inv_bit ^ inv_sub).to(tl.uint32)

    out_ptrs = offs_m[:, None] * stride_ym + offs_y_n[None, :]
    tl.store(CMP + out_ptrs, cmp_u32, mask=mask_m)
    tl.store(INV_BIT + out_ptrs, inv_bit, mask=mask_m)
    tl.store(INV_SUB + out_ptrs, inv_sub, mask=mask_m)
    tl.store(DELTA + out_ptrs, delta, mask=mask_m)
...
RECIPES = [{
    "name": "topk_forward_bisect_m2_bitop3_nosel_probe",
    "kernel_fn": _topk_forward_bisect_m2_bitop3_nosel_probe,
    "kernel_symbol": "_topk_forward_bisect_m2_bitop3_nosel_probe",
    "signature": {
        "X": "*bf16",
        "stride_xm": "i32",
        "CMP": "*u32",
    ...
    "harness_constants": {"N_COLS": _N_COLS, "N_EXPTS_ACT_HC": _N_EXPTS_ACT},
    "scalar_args": {
        "stride_xm": "N_COLS",
    ...
    "outputs": [
        ...
        {"name": "INV_BIT", "dtype": "u32", "elems": "N_ROWS * N_EXPTS_ACT_HC"},
        {"name": "INV_SUB", "dtype": "u32", "elems": "N_ROWS * N_EXPTS_ACT_HC"},
        {"name": "DELTA", "dtype": "u32", "elems": "N_ROWS * N_EXPTS_ACT_HC"},
    ],
