"""Probe cmp + arithmetic inversion only (inv_sub = 1 - cmp)."""
...
@triton.jit
def _topk_forward_bisect_m2_invsub_probe(
    X, stride_xm,
    CMP, INV_SUB, stride_ym,
    n_rows, n_expts_tot,
...
RECIPES = [{
    "name": "topk_forward_bisect_m2_invsub_probe",
    "kernel_fn": _topk_forward_bisect_m2_invsub_probe,
    "kernel_symbol": "_topk_forward_bisect_m2_invsub_probe",
    "signature": {
        "X": "*bf16",
        "stride_xm": "i32",
        "CMP": "*u32",
    ...
    "constexprs": {"BLOCK_M": 32, "N_EXPTS_PAD": 128, "N_EXPTS_ACT": 4, "BLOCK_N": 32},
    ...
    "inputs": [{
        "name": "X", "dtype": "bf16", "elems": "N_ROWS * N_COLS", "range_lo": -4.0, "range_hi": 4.0,
    }],
    "outputs": [
